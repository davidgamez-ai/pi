import {
    DEBUG_COMPLEXITY,
    DEBUG_CRYSTALLIZED_INTELLIGENCE,
    DEBUG_FLUID_INTELLIGENCE, DEBUG_MAX_INTELLIGENCE,
    DEBUG_PREDICTION_MATCH, DEBUG_PROBABILITY, MEASURE_PERFORMANCE
} from "./Debug.js";
...
import {Compressibility, DataSet, Prediction} from "./Types.js";
...
export class IntelligenceEngine {
    ...
    updateCrystallizedIntelligence(){
        ...
        for(let ds:number = 0; ds<this.data.dataSets.length; ++ds){
            ...
            const timeWindow:number = this.ml.getTimeWindow();

            //Work through original data and predictions
            //Need to offset original data by time window.
            for(let i:number=0; i<dataSet.predictions.length; ++i){
                ...
                const prediction:Prediction = dataSet.predictions[i];//Local reference for convenience
                if(DEBUG_PREDICTION_MATCH) console.log("Prediction Match. Actual data. TimeStamp: " + dataSet.tsData[i+timeWindow].timeStamp + "; value: " + dataSet.tsData[i+timeWindow].value);

                let pm:number = this.getPredictionMatch(dataSet.tsData[i+timeWindow].value, prediction.mean, prediction.standardDeviation);
                if(DEBUG_PREDICTION_MATCH) console.log("Prediction Match. Prediction. predictionMean: " + prediction.mean + "; prediction variance: " + prediction.standardDeviation + "; prediction match: " + pm);

                //Random guess allocates an equal probability to all numbers in the range of the dataset
                const randomPredictionProbability:number = this.getRandomPredictionProbability(dataSet);

                //Subtract random guess
                // Set to zero if less than zero - this is different from the agent case
                let finalPM:number = pm - randomPredictionProbability;
                if(finalPM < 0)
                    finalPM = 0;
        ...
        if(MEASURE_PERFORMANCE) Timer.end("MachineLearning", true);
    }
     ...
     * */
    calculateCrystallizedIntelligence(pmStrings:string[][]):number{
        ...
        for(let ds:number=0; ds<this.data.dataSets.length; ++ds) {
            //Local reference for convenience
            const dataSet:DataSet = this.data.dataSets[ds];
            ...
            let dsPM:number = 0, dsString:string = "";
            for(let p:number=0; p<this.pmYArray[ds].length; ++p) {
                dsPM += this.pmYArray[ds][p];
                dsString += pmStrings[ds][p];
            }
            if(DEBUG_COMPLEXITY) console.log("Complete data length: " + completeData.length);

            //Multiply total by prediction compression ratio
            let predCom:number = Util.getPMCompRatio(dsString);
            dsPM *= predCom;
            ...
            let dsValues:number[] = dataSet.tsData.map(val => (val.value));
            let dsEnvComp:number = Util.getEnvironmentCompRatio(dsValues);
            dsPM *= dsEnvComp;
            ...
            if (DEBUG_CRYSTALLIZED_INTELLIGENCE) console.log(`calculateCrystallizedIntelligence: Dataset CI: ${dsCI}; Environment compression: ${dsEnvComp}; prediction compression: ${predCom}.`);
    ...
    calculateMaxCrystallizedIntelligence(){
        ...
        for(let dataSet of this.data.dataSets){
            ...
            let dsPredictions:Prediction[] = dataSet.tsData.map(item => ({
            ...
            for(let pred of dsPredictions){
                ...
                let pm:number = this.getPredictionMatch(pred.mean, pred.mean, pred.standardDeviation);
            ...
            let predComp:number = Util.getPMCompRatio(predictionString);
            dsPM *= predComp;
            ...
            dsPM *= dsEnvComp;
            if(DEBUG_COMPLEXITY) console.log(dataSet.name + ". Prediction compressibility: " + predComp + " data environment compressibility: " + dsEnvComp + "; pm max for data set: " + dsPM);

            /* Round down to zero or take log2 */
            let dsMaxCI:number = 0;
            if(dsPM >= 1)
                dsMaxCI = Math.log2(dsPM);

            //Store max CI for this environment
            this.dataSetCIMax.push(dsMaxCI);
            if (DEBUG_CRYSTALLIZED_INTELLIGENCE) console.log(`calculateMaxCrystallizedIntelligence: Dataset Max CI: ${dsMaxCI}; Environment compression: ${dsEnvComp}; prediction compression: ${predComp}.`);
        }

        //Next take compression of all environments taken together into account
        this.totalEnvironmentCompressibility = Util.getEnvironmentCompRatio(completeData);
        if(DEBUG_MAX_INTELLIGENCE) console.log("Total environment compressibility: " + this.totalEnvironmentCompressibility);
        ...
        if(DEBUG_MAX_INTELLIGENCE){
            ...
            for(let ds:number=0; ds<this.data.dataSets.length; ++ds){
                let dataSet:DataSet = this.data.dataSets[ds];
                const ranPredProb:number = this.getRandomPredictionProbability(dataSet)
     ...
     *  */
    getPredictionMatch(value:number, mean:number, standardDeviation:number):number{
        if(DEBUG_PROBABILITY) console.log("getProbability> value: " + value + "; mean: " + mean + "; standard deviation: " + standardDeviation);
     ...
     * */
    getRandomPredictionProbability(dataSet:DataSet):number{
        return 0;
    }
     ...
     */
    updateFluidIntelligence():void {
        //Clear arrays
        this.fiXArray = [];
        this.fiYArray = [];

        //Work through crystalized intelligence array for points within polynomial window
        for(let i:number = this.polynomialWindow; i < this.ciXArray.length-this.polynomialWindow; ++i) {
            //Get the polynomial approximating the window at this point
            let terms:Array<number> = this.getPolynomialTerms(i);

            //Calculate the slope, which corresponds to the fluid intelligence
            let slope: number = this.getSlope(terms, i);
     ...
     * @param x
     */
    getPolynomialTerms(x:number):Array<number>{
     ...
     * @param x
     */
    calculatePolynomialResult(terms:Array<number>, x:number):number{
        let result:number = 0;
    ...
    getSlope(terms:Array<number>, x:number):number{
        ...
        if(DEBUG_FLUID_INTELLIGENCE) console.log("IEngine. Differential terms: " + JSON.stringify(differential));

        //Use the terms to calculate the slope and return it
        let slope:number = this.calculatePolynomialResult(differential, x);
        if(DEBUG_FLUID_INTELLIGENCE) console.log("IEngine. Slope: " + slope);
        return slope;
    }
     ...
     * @param xEnd
     */
    getPolynomialData(xStart:number, xEnd:number):Array<{x:number, y:number}>{
    ...
    getTotalCrystallizedIntelligencePlot():[number[], number[]]{
    ...
    getDataSetCrystallizedIntelligencePlot():[number[][], number[][]]{
        return [this.dsCIXArray, this.dsCIYArray];
    }


    /** Returns x and y values for fluid intelligence plot */
    getFluidIntelligencePlot():[number[], number[]]{
