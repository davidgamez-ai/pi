import {StateDist, IMeasure, IMeasureStore, IMeasurePlot} from "./DataTypes.js";
...
export class IntelligenceEngine {
    ...
    ciTotalXArray:number[] = [];
    ciTotalYArray:number[] = [];
    ...
    constructor(mazeManager:MazeManager, complexity:Complexity){
    ...
    getMatch(predDist:StateDist, actualDist:StateDist):[number,number] {
        if(DEBUG_CRYSTALLIZED_INTELLIGENCE) console.log("INTELLIGENCE. Prediction: " + JSON.stringify(predDist) + "; Actual: " + JSON.stringify(actualDist));

        //Calculate the total distance between the two probability distributions
        let matchPredActual:number = Prob.totalMatch(predDist, actualDist);

        //If the agent guesses randomly it will assign equal probability to all states
        let randomDist:StateDist = Util.newStateDist(1.0/3.0);

        //Get the distance of the random guess from the actual state
        let matchRandomActual:number = Prob.totalMatch(randomDist, actualDist);

        //Subtract random guess distance from the distance of the predictions
        let finalMatch:number = matchPredActual - matchRandomActual;

        //Calculate best possible match
        //A perfect match would be 4, so subtract random match from perfect match
        let bestMatch:number = 4-matchRandomActual;
        if (DEBUG_CRYSTALLIZED_INTELLIGENCE) console.log("INTELLIGENCE. Hellinger distances: pred vs actual: " + matchPredActual + "; random vs actual: " + matchRandomActual + "; finalMatch: " + finalMatch + "; bestMatch: " + bestMatch );
    ...
    addMeasurement(timeStamp:number, mazeID:string, iMeasure:IMeasure){
     ...
     * @param mazeID
     */
    getMeasurementCount(timeStamp:number, mazeID:string):number {
        if(this.iMeasurements[timeStamp] === undefined)
            return 0;
        if(this.iMeasurements[timeStamp][mazeID] === undefined)
            return 0;
     ...
     * */
    updateIntelligence(timeStamp:number){
        ...
        let allMazesPMTotal:number=0, allMazesMaxPMTotal=0;

        //Work through maze IDs
        const mazeIDsArray:string[] = [];//Store the IDs of the mazes at this time stamp for compression calculation
        for(let mazeID in this.iMeasurements[timeStamp] ){
            if(DEBUG_CRYSTALLIZED_INTELLIGENCE) console.log(`Calculating intgelligence for maze ${mazeID}`);
            mazeIDsArray.push(mazeID);

            //Work through measurements for this maze
            let mazePMTotal:number=0, maxMazePMTotal:number=0;
            for(let measure of this.iMeasurements[timeStamp][mazeID]) {
                ...
                if(DEBUG_MAX_CI) console.log("predDistance: " + measure.predMatch + "; maxDist: " + measure.maxMatch +  "; predCompress: " + measure.predCompress + "; actual compress: " + measure.actualCompress);
            }
            ...
            const mazeCI:number = this.calculateCrystallizedIntelligence(mazePMTotal, mazeCompressionRatio);

            //Calculate maximum CI for this maze at this timestamp
            const maxMazeCI:number = this.calculateCrystallizedIntelligence(maxMazePMTotal, mazeCompressionRatio);
        ...
        let allMazesCompressibility:number = this.complexity.getMazesCompressionRatio_LZUTF8(mazeIDsArray);

        //Multiply the total intelligence by the compressibility of the combined environments
        let finalCI:number = this.calculateCrystallizedIntelligence(allMazesPMTotal, allMazesCompressibility)
        let finalMaxCI:number = this.calculateCrystallizedIntelligence(allMazesMaxPMTotal, allMazesCompressibility);
        ...
        if(DEBUG_CRYSTALLIZED_INTELLIGENCE) console.log("Crystallized intelligence: " + finalCI + "; Max CI: "+ finalMaxCI);
    ...
    calculateCrystallizedIntelligence(pmSum:number, compressionRatio:number){
        //Multiply the total prediction match by the compressibility of the environment(s)
        let ci:number = pmSum * compressionRatio;
    ...
    updateFluidIntelligence():void {
        ...
        for(let i:number = this.polynomialWindow; i < this.ciTotalXArray.length-this.polynomialWindow; ++i) {
