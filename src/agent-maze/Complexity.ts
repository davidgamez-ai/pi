import {StateDist} from "./DataTypes.js";
...
export class Complexity{
    ...
    predictionComplexityCanvas:HTMLCanvasElement;
    mazeComplexityCanvas:HTMLCanvasElement;

    //Compression levels for image compression
    maxCompressionLevel:number = 1;
    minCompressionLevel:number = 0.1;

    //Maze manager
    mazeManager:MazeManager;

    constructor(mazeManager:MazeManager) {
        //Store reference to maze manager.
        this.mazeManager = mazeManager;

        //Store references to canvases for calculating prediction and maze complexity
        this.predictionComplexityCanvas = <HTMLCanvasElement> document.getElementById(PREDICTION_COMPLEXITY_CANVAS);
        if(this.predictionComplexityCanvas === null)
            throw "Prediction complexity canvas not found!";
        this.mazeComplexityCanvas = <HTMLCanvasElement> document.getElementById(MAZE_COMPLEXITY_CANVAS);
    ...
    getCompressionRatio_Image(stateDist:StateDist):number{
        //Get context
        ...
        let maxCompLength:number =dataURI.substring(dataURI.indexOf(',') + 1, dataURI.length).length;
        ...
        let minCompLength:number =dataURI.substring(dataURI.indexOf(',') + 1, dataURI.length).length;

        if(DEBUG_COMPLEXITY) console.log("COMPLEXITY. Max canvas data length " +  maxCompLength + ". Min canvas data length " +  minCompLength + "; ratio: " + minCompLength/maxCompLength);
     ...
     * */
    getStateDistCompressionRatio_LZUTF8(stateDist:StateDist):number{
        let str:string = Util.getStateDistComplexityString(stateDist);
        ...
        if(DEBUG_COMPLEXITY) console.log("COMPLEXITY. StateDist with LZUTF8. string: " + str + "; output: " + output + "; compression ratio: " + output.length/str.length);
     ...
     * @param mazeIdxArr
     */
    getMazesCompressionRatio_LZUTF8(mazeIDs:string[]):number{
        ...
        let compressedMazeStr:string = LZUTF8.compress(mazeStr);

        if(DEBUG_COMPLEXITY) console.log("COMBINED MAZE COMPLEXITY. Compressed length: " + compressedMazeStr.length + "; uncompressed length: " +  mazeStr.length + "; ratio: " + (compressedMazeStr.length  / mazeStr.length));
...
function getTestStateDist(index:number):StateDist{
