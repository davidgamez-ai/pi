...
import {MazeMap} from "./DataTypes.js";
...
export class Maze {
    ...
    mazeMap:MazeMap;//Only assigned when maze is loaded
    ...
    wallColor:string = "rgb(0, 0, 0)";
    gridColor:string = "rgb(100, 100, 100)";
    rewardColor:string = "rgb(255, 0, 0)";
    ...
    constructor(mazeMap:MazeMap){
        ...
        if(DEBUG_MAZE) console.log("MAZE. ID: " + this.mazeMap.id + "; Canvas: (" + this.canvas.width + ", " + this.canvas.height + "). Grid size: " + this.gridSize);
        if(DEBUG_MAZE) console.log(`Maze width ${this.width}; height: ${this.height}`);
    }
    ...
    isReward(x:number, y:number):boolean{
        //Check that agent position is not a wall
    ...
    draw(){
        ...
        for(let y:number=0; y<this.mazeMap.rows.length; ++y){
            for(let x:number=0; x<this.mazeMap.rows[y].length; ++x){
                let tmpX:number = x*this.gridSize;
                let tmpY:number = y*this.gridSize;
    ...
    drawMazeBackground(ctx:any, x:number, y:number){
        if(DEBUG_MAZE) console.log("Maze background: x: " + x + "; y: " + y + "; w/h: " + this.gridSize);
        ctx.beginPath();
        ctx.lineWidth = "1";
    ...
    drawMazeBlock(ctx:any, x:number, y:number){
        if(DEBUG_MAZE) console.log("Maze block: x: " + x + "; y: " + y + "; w/h: " + this.gridSize);
    ...
    /** Draws a reward */
    drawReward(ctx:any, x:number, y:number){
