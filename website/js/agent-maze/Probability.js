 ...
 * @returns {number}
 */
export function kLDivergence(P, Q, decimals) {
...
export function totalMatch(stateDist1, stateDist2) {
    let matchTot = 0;
...
export function hellingerDistance(P, Q, decimalPlaces) {
    //Check that both probability distributions are valid
    ...
    let result = Math.pow(2, -0.5) * Math.sqrt(sum);
    return result;
}
 ...
 * @param {number} decimals
 */
export function checkDistributions(distArr, decimalPlaces) {
    distArr.forEach(dist => {
        let probSum = sumProbs(dist, decimalPlaces);
...
export function multiplyDistributions(X, Y, decimals) {
    let newProbDist = {};
...
export function getProbabilityDistributions(stateTransitionMap) {
    ...
    for (let smKey in stateTransitionMap) {
        let sArray = stateTransitionMap[smKey]; //Local reference to array
        //Holds counts of the occurrence of ech state in array
        let stateCount = Util.newStateDist(0);
        //Probability distributions of predictions about next state.
        let stateProbs = Util.newStateDist(0);
        ...
        for (let sKey in stateCount) {
            let sum = stateCount[sKey].e + stateCount[sKey].w + stateCount[sKey].r;
