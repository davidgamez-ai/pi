...
import { NULL, EMPTY, WALL, REWARD, UP, DOWN, LEFT, RIGHT, MAZE_CANVAS, MAZE_SELECT_COMBO, MAZE_SELECT_BUTTON, START_EXPLORE_MAZE_BUTTON, AGENT_LEARNING_CHECKBOX, STOP_EXPLORE_MAZE_BUTTON, RECALCULATE_INTELLIGENCE_SPAN, RECALCULATE_INTELLIGENCE_INTERVAL_INPUT, DEFAULT_ICALC_FREQUENCY, RESET_AGENT_BUTTON, MODAL_MAZE_LIST, MAZE_SELECT_MODAL, CLOSE_MAZE_SELECT_BUTTON, MAZE_SELECT_FEEDBACK, PLOT_CI_INDIVIDUAL_MAZES_CHECKBOX, CLEAR_GRAPH_SELECTION_BUTTON, MAZE_COMPRESSIBILITY_MODAL, SHOW_MAZE_COMPRESSIBILITY_BUTTON, CLOSE_MAZE_COMPRESSIBILITY_BUTTON, TOTAL_MAZE_COMPRESSIBILITY, MAZE_COMPRESSIBILITY } from './Globals.js';
...
export class AgentMain {
    ...
    mazeSelectCombo;
    mazeSelectButton;
    startExploreMazeButton;
    agentLearningCB;
    recalcIntelSpan;
    plotCIIndividualMazesCB;
    ...
    buildModals() {
        ...
        const modalMazeList = DomUtil.getDiv(MODAL_MAZE_LIST);
        ...
        const showCompressibilityButton = DomUtil.getButton(SHOW_MAZE_COMPRESSIBILITY_BUTTON);
        showCompressibilityButton.onclick = async (event) => {
            mazeCompressibilityModal.style.display = "block";
        };
        //Close compressibility modal
        const closeCompressibilityModalButton = DomUtil.getButton(CLOSE_MAZE_COMPRESSIBILITY_BUTTON);
        ...
        const mazeIDs = this.mazeManager.getMazeIDs();
        let compHTMLStr = "";
        ...
        const totalCom = this.complexity.getMazesCompressionRatio_LZUTF8(mazeIDs);
    ...
    addModalListeners() {
        //Maze modal
        const mazeSelectModal = DomUtil.getDiv(MAZE_SELECT_MODAL);
        //Area for feedback to user
        const mazeSelectFeedback = DomUtil.getDiv(MAZE_SELECT_FEEDBACK);
        //Launch maze select modal
        const mazeSelectButton = DomUtil.getButton(MAZE_SELECT_BUTTON);
        ...
        const closeDataSelectModalButton = DomUtil.getButton(CLOSE_MAZE_SELECT_BUTTON);
    ...
    addSelectListeners() {
        ...
        if (this.mazeSelectCombo !== null) {
            //Add options
            for (let i = 0; i < this.mazeManager.size(); i++) {
                const maze = this.mazeManager.getByIndex(i);
                this.mazeSelectCombo.options[i] = new Option(maze.getName(), maze.getId().toString());
            }
            //Listen for changes
            this.mazeSelectCombo.onchange = event => {
                if (DEBUG_EVENTS)
                    console.log("EVENTS. Maze change. New maze id: " + this.mazeSelectCombo.value);
    ...
    addCustomEventListeners() {
        ...
        document.addEventListener('UpdateIntelligence', event => {
            if (DEBUG_EVENTS)
                console.log("EVENTS. UpdateIntelligence event received.");
    ...
    addInputListeners() {
        let recalcIntelIntervalInput = DomUtil.getInput(RECALCULATE_INTELLIGENCE_INTERVAL_INPUT);
        recalcIntelIntervalInput.value = DEFAULT_ICALC_FREQUENCY.toString();
        recalcIntelIntervalInput.addEventListener('input', event => {
            let input = event.target;
            if (input !== null) {
                if (DEBUG_EVENTS)
                    console.log("Setting intelligence calculation interval: " + input.value);
    ...
    addButtonListeners() {
        ...
        let stopExploreMazeButton = DomUtil.getButton(STOP_EXPLORE_MAZE_BUTTON);
        ...
        //Stop full maze exploration
        let resetAgentButton = DomUtil.getButton(RESET_AGENT_BUTTON);
        ...
        let clearGraphSelectionButton = DomUtil.getButton(CLEAR_GRAPH_SELECTION_BUTTON);
    ...
    updateMazeSelection() {
        ...
        const mazeSelectCombo = DomUtil.getSelect(MAZE_SELECT_COMBO);
        //Clear list of mazes
