...
export class Agent {
    ...
    disableIntelligenceUpdate = false;
    //Number of changes before update of intelligence calculations
    iCalcFreq = DEFAULT_ICALC_FREQUENCY;
    //Measures the intelligence of the agent
    intelligenceEngine = {}; //Has to be set after constructor
    ...
    canvas;
    //Colours for drawing.
    agentBorderColor = "rgb(0, 0, 0)";
    agentFillColor = "rgb(0, 255, 0)";
    agentDirectionColor = "rgb(255, 150, 255)";
    sensorBackgroundColor = "rgb(0, 0, 255)";
    sensorTextColor = "rgb(255, 255, 255)";
    ...
    setLocation(x, y) {
        ...
        this.updateSensors();
        if (DEBUG_AGENT)
            console.log("AGENT. Set Agent Position: x:" + x + "; y: " + y);
    }
    ...
    setDirection(d) {
        if (DEBUG_AGENT)
            console.log("AGENT. setDirection(). current d: " + this.state.d + "; new d: " + d);
        ...
        this.updateSensors();
        if (DEBUG_AGENT)
            console.log("AGENT. New agent direction: d:" + d);
    }
    ...
    rotate(d) {
        if (DEBUG_AGENT)
            console.log("AGENT. rotate(). current d: " + this.state.d + "; new d: " + d);
    ...
    move() {
        if (DEBUG_AGENT)
            console.log("AGENT. move(). current x: " + this.x + "; current y: " + this.y);
        ...
        this.updateSensors();
        if (DEBUG_AGENT)
            console.log("AGENT. move(). Move status: " + moved + "; new x: " + this.x + "; new y: " + this.y);
        //Keep track of number of changes for intelligence update
        if (!this.disableIntelligenceUpdate)
            this.changeCount++;
    ...
    updateProbabilities(changeKey) {
        ...
        if (DEBUG_PROBABILITY)
            console.log("AGENT. Direction change. Key: " + changeKey + "' Total number of keys: " + Object.keys(this.stateTransitionMap).length + "; number of entries for this key: " + this.stateTransitionMap[changeKey].length);
    ...
    draw() {
        ...
        if (DEBUG_AGENT)
            console.log("AGENT: draw(). x: " + this.x + "; y: " + this.y + "; startX: " + startX + "; startY: " + startY);
    ...
    drawSensor(ctx, label, size, fontSize) {
        ctx.save();
    ...
    updateSensors() {
        ...
        if (DEBUG_SENSORS)
            console.log("SENSORS. Location: (" + this.x + "," + this.y + "). State: " + Util.getStateMapKey(this.state));
    }
    ...
    setIntelligenceEngine(intelligenceEngine) {
