...
export class Plotter {
    ...
    plotIndividualMazes = true;
    constructor(intelligenceEngine, mazeManager) {
    ...
    loadMazeLegends() {
        ...
        const colorArray = Colors.getRandomColors(this.mazeManager.size());
    ...
    setCIClickX(x) {
        this.ciClickX = x;
        if (DEBUG_EVENTS)
            console.log("EVENTS. setCIClickX. piClickX: " + x);
    ...
    plotMatch() {
        ...
        if (DEBUG_GRAPHS)
            console.log("Prediction match plotting complete.");
    }
    /** Builds and returns a trace for the prediction match graph */
    getPMTrace(xArr, yArr, zArr, mazeID) {
    ...
    plotCrystallizedIntelligence() {
        if (DEBUG_GRAPHS)
            console.log("Plotting predictive intelligence.");
        //Build traces
        ...
        if (DEBUG_GRAPHS)
            console.log("Intelligence plotting complete. Plot count: " + this.plotCount);
    }
    ...
    addGraphEventListeners() {
        ...
        let myPlot = DomUtil.getDiv(CRYSTALLIZED_INTELLIGENCE_GRAPH_DIV);
        if (myPlot !== null) {
            myPlot.on('plotly_click', (event) => {
                if (event.points !== null && event.points.length > 0) {
                    ...
                    this.plot();
                    if (DEBUG_EVENTS)
                        console.log("User clicked on Crystallized Intelligence Graph, x position: " + this.ciClickX);
                }
            });
        }
    }
