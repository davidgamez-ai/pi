...
export class MachineLearning {
    ...
    buildModels() {
        ...
        let numFeatures = 1;
        //Create models
        for (let m = 0; m < this.numberOfModels; ++m) {
    ...
    async train() {
        ...
        for (let model = 0; model < this.modelArray.length && !this.stopTraining; ++model) {
            ...
            await this.modelArray[model].fit(inputTrain, labelTrain, {
                ...
                callbacks: {
                    ...
                    onEpochEnd: (epoch, log) => {
                        if (DEBUG_TRAINING)
                            console.log("On epoch end: " + epoch + " epoch counter: " + epochCounter);
                    ...
                    onBatchEnd: (batch, log) => {
                        if (DEBUG_TRAINING)
                            console.log("On batch end: " + (batch + epochCounter * 10));
    ...
    predict() {
        ...
        for (let i = 0; i < this.sequenceDataArray.length; ++i) {
            //Get tensors
            ...
            let total = 0;
            let finalPredArr = [];
            for (let p = 0; p < predictionArray[0].length; ++p) { //Work through predictions
                const predictionValuesArray = [];
                for (let m = 0; m < predictionArray.length; ++m) { //Work through predictions for each model
                    total += predictionArray[m][p];
                    predictionValuesArray.push(predictionArray[m][p]);
                }
                //Calculate mean and SD
                const mean = Stats.mean(predictionValuesArray);
                const variance = Stats.sampleStandardDeviation(mean, predictionValuesArray);
                //Store prediction
                finalPredArr.push({
                    mean: mean,
            ...
            */
            let tmpTimeSeries = this.data.dataSets[i].tsData.slice(this.timeWindow, this.data.dataSets[i].tsData.length);
            if (DEBUG_DATA)
                console.log(`Length of prediction array: ${finalPredArr.length}; length of raw data corresponding to predictions: ${tmpTimeSeries
