...
import { BATCH_LOSS_GRAPH_DIV, EPOCH_LOSS_GRAPH_DIV, DATA_GRAPH_DIV, PREDICTION_MATCH_GRAPH_DIV, CRYSTALLIZED_INTELLIGENCE_GRAPH_DIV, FLUID_INTELLIGENCE_GRAPH_DIV, PREDICTION_PROBABILITY_GRAPH_DIV } from "./Constants.js";
import { DEBUG_DATA_GRAPH, DEBUG_EVENTS, DEBUG_FLUID_INTELLIGENCE, DEBUG_INTELLIGENCE_GRAPHS, DEBUG_TRAINING_GRAPHS } from "./Debug.js";
...
export class Plotter {
     ...
     */
    epochValLoss = []; //Each model has its own set of values for the epochs
    epochLoss = []; //Each model has its own set of values for the epochs
    epochLossTraceColors = []; //Separate trace colour for each model
    epochValLossTraceColors = []; //Separate trace colour for each model
    //Plots individual traces for each data set
    plotIndividualDatasets = true;
    ...
    plotData() {
        ...
        const dataSet = this.data.dataSets[this.data.dataIndex];
    ...
    plotPredictionMatch() {
        ...
        if (this.dataClickIndex !== -1) {
            let xTS = this.data.dataSets[this.data.dataIndex].tsData[this.dataClickIndex].timeStamp;
            let yVal = 0;
            //Find corresponding prediction match - not efficient, but does the job
            let yValFound = false;
            for (let i = 0; i < this.iEngine.pmXArray[this.data.dataIndex].length; ++i) {
    ...
    plotPredictionDistribution() {
        ...
        const clickTS = dataSet.tsData[this.dataClickIndex].timeStamp;
        //Get prediction for this timestamp
        let pFound = false, pMean = 0, pVariance = 0;
        ...
        //Get max and min x
        let maxX = Util.getMax(pXArray);
        let minX = Util.getMin(pXArray);
        //Build random plot
        const randomPredictionProbability = this.iEngine.getRandomPredictionProbability(dataSet);
        const rXArray = [minX, minX, maxX, maxX];
        const rYArray = [0, randomPredictionProbability, randomPredictionProbability, 0];
     ...
     * @param numSD
     */
    getNormalData(mean, variance, numSD) {
        let xArray = [], yArray = [];
        for (let x = mean - numSD * variance; x <= mean + numSD * variance; x += variance / 10) {
            xArray.push(x);
    ...
    plotEpochLoss() {
        if (DEBUG_TRAINING_GRAPHS)
            console.log(this.epochValLoss);
        if (DEBUG_TRAINING_GRAPHS)
            console.log(this.epochLoss);
        //Run some checks
        ...
        //Build x array
        let xArr = [];
        for (let epoch = 0; epoch < this.epochLoss[0].length; ++epoch)
            xArr.push(epoch);
        let traceArray = [];
        for (let model = 0; model < this.ml.getNumberOfModels(); ++model) {
            //Add loss
            traceArray.push({
                x: xArr,
    ...
    addBatchLoss(model, batch, loss) {
        this.batchX.push(batch);
        this.batchLoss.push(loss);
    }
    addEpochLoss(model, epoch, loss, valLoss) {
    ...
    plotCrystallizedIntelligence() {
        if (DEBUG_INTELLIGENCE_GRAPHS)
            console.log("Plotting crystallized intelligence.");
        this.show(CRYSTALLIZED_INTELLIGENCE_GRAPH_DIV);
        //Get arrays for crystallized intelligence plot summed across all data sets
        let [xArr, yArr] = this.iEngine.getTotalCrystallizedIntelligencePlot();
        //Get arrays for crystallized intelligence plot calculated independently for each data set.
        let [dsXArr, dsYArr] = this.iEngine.getDataSetCrystallizedIntelligencePlot();
        //Build traces
        let traceArray = [];
        ...
        if (this.polynomialTerms.length > 0 && this.ciClickX > 0) {
            ...
            let xStart = this.ciClickX - this.iEngine.polynomialWindow;
            let xEnd = this.ciClickX + this.iEngine.polynomialWindow;
            let xPolyArray = [], yPolyArray = [];
            ...
            let slope = this.iEngine.getSlope(this.polynomialTerms, this.ciClickX);
            //Calculate slope using c = y-mx
            let yIntersect = this.iEngine.calculatePolynomialResult(this.polynomialTerms, this.ciClickX) - slope * this.ciClickX;
        ...
        if (DEBUG_INTELLIGENCE_GRAPHS)
            console.log("Crystallized intelligence plotting complete. Plot count: " + this.plotCount);
    }
    /** Plots the fluid intelligence */
    plotFluidIntelligence() {
        if (DEBUG_INTELLIGENCE_GRAPHS)
            console.log("Plotting intelligence rate of change.");
        ...
        if (DEBUG_INTELLIGENCE_GRAPHS)
            console.log("Intelligence change plotting complete.. Plot count: " + this.plotCount);
    }
    /** Adds listeners for graph events in plotly */
    addGraphEventListeners() {
        //PI graph event handling for click events
        let ciPlot = document.getElementById(CRYSTALLIZED_INTELLIGENCE_GRAPH_DIV);
        if (ciPlot !== null) {
            ciPlot.on('plotly_click', (event) => {
                if (event.points !== null && event.points.length > 0) {
                    ...
                    if (DEBUG_FLUID_INTELLIGENCE)
                        console.log(this.polynomialTerms);
                    this.replot();
                    if (DEBUG_EVENTS)
                        console.log("User clicked on RoT Intelligence Graph, x position: " + this.ciClickX);
                }
            });
        }
        //Enables user to click on data plot to see how prediction match was calculated
        let dataPlot = document.getElementById(DATA_GRAPH_DIV);
        if (dataPlot !== null) {
            dataPlot.on('plotly_click', (event) => {
                if (event.points !== null && event.points.length > 0) {
    ...
    setData(data) {
        ...
        const dataStore = data.getDataStore();
