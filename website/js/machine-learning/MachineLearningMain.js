...
import { BATCH_SIZE_INPUT, BUILD_BUTTON, CLEAR_TRAINING_BUTTON, CRYSTALLIZED_INTELLIGENCE_GRAPH_DIV, DATA_SELECT, EPOCH_INPUT, LSTM_UNITS_INPUT, NUM_MODELS_INPUT, RESET_BUTTON, TEST_PERCENT_INPUT, TIME_WINDOW_INPUT, START_TRAINING_BUTTON, STOP_TRAINING_BUTTON, TRAIN_PERCENT_INPUT, TRAINING_CYCLES_INPUT, VALIDATION_PERCENT_INPUT, PREDICTION_PROBABILITY_GRAPH_DIV, PREDICTION_MATCH_GRAPH_DIV, FLUID_INTELLIGENCE_GRAPH_DIV, DATA_GRAPH_DIV, RECALCULATE_INTELLIGENCE_BUTTON, SELECT_DATA_SETS_BUTTON, DATA_SET_SELECTION_MODAL, MODAL_DATA_SET_LIST, CLOSE_DATA_SELECT_BUTTON, COMPRESSIBILITY_MODAL, CLOSE_COMPRESSIBILITY_MODAL_BUTTON, SHOW_COMPRESSIBILITY_MODAL_BUTTON, TOTAL_COMPRESSIBILITY_SPAN, DATA_SET_COMPRESSIBILITY_DIV, DATA_SPLIT_SLIDER, CLEAR_GRAPH_SELECTION_BUTTON, PLOT_CI_INDIVIDUAL_DATASETS_CB, P_VALUE_SELECT, PLOT_BATCH_LOSS_CB } from "./Constants.js";
...
export class MachineLearningMain {
    ...
    buildModals() {
        ...
        const dataSetList = DomUtil.getDiv(MODAL_DATA_SET_LIST);
        if (dataSetList === null)
            throw "Data set list div not found.";
        //Get complete set of data
        const dataStore = this.data.getDataStore();
    ...
    addCheckBoxListeners() {
        const plotIndividualDataSetsCB = DomUtil.getInput(PLOT_CI_INDIVIDUAL_DATASETS_CB);
        ...
        const plotBatchLossCB = DomUtil.getInput(PLOT_BATCH_LOSS_CB);
    ...
    addModalListeners() {
        //Data modal
        const dataSelectModal = DomUtil.getDiv(DATA_SET_SELECTION_MODAL);
        //Launch data select modal
        const selectDataSetsButton = DomUtil.getButton(SELECT_DATA_SETS_BUTTON);
        selectDataSetsButton.onclick = async (event) => {
            dataSelectModal.style.display = "block";
        };
        //Close data selection modal
        const closeDataSelectModalButton = DomUtil.getButton(CLOSE_DATA_SELECT_BUTTON);
        ...
        const compressibilityModal = DomUtil.getDiv(COMPRESSIBILITY_MODAL);
        //Launch compressibility modal
        const showCompressibilityButton = DomUtil.getButton(SHOW_COMPRESSIBILITY_MODAL_BUTTON);
        showCompressibilityButton.onclick = async (event) => {
            compressibilityModal.style.display = "block";
        };
        //Close compressibility modal
        const closeCompressibilityModalButton = DomUtil.getButton(CLOSE_COMPRESSIBILITY_MODAL_BUTTON);
    ...
    updateDataSelection() {
        ...
        const dataSelect = DomUtil.getSelect(DATA_SELECT);
        ...
        for (let i = 0; i < dataCheckBoxes.length; ++i) {
            let checked = dataCheckBoxes[i].checked;
            if (checked) {
        ...
        if (MEASURE_PERFORMANCE)
            this.data.printStatistics();
        //Plot graph of first item on new list.
        this.plotDataGraph();
    }
    /** Adds button listeners for application */
    addButtonListeners() {
        const startTrainingButton = DomUtil.getButton(START_TRAINING_BUTTON);
        startTrainingButton.onclick = async (event) => {
            ...
            for (let t = 0; t < this.ml.trainingCycles && !this.ml.stopTraining; ++t) {
        ...
        const stopTrainingButton = DomUtil.getButton(STOP_TRAINING_BUTTON);
        stopTrainingButton.onclick = async (event) => {
            this.ml.stopTraining = true;
            Logger.info("Waiting for training to stop");
        };
        const clearTrainingButton = DomUtil.getButton(CLEAR_TRAINING_BUTTON);
        ...
        const buildButton = DomUtil.getButton(BUILD_BUTTON);
        ...
        const resetButton = DomUtil.getButton(RESET_BUTTON);
        ...
        const recalcIntelButton = DomUtil.getButton(RECALCULATE_INTELLIGENCE_BUTTON);
        ...
        const clearGraphSelectionButton = DomUtil.getButton(CLEAR_GRAPH_SELECTION_BUTTON);
    ...
    addCustomEventListeners() {
        ...
        document.addEventListener('UpdateCompressibility', event => {
            if (DEBUG_EVENTS)
                console.log("EVENTS. UpdateCompressibility event received.");
    ...
    addSelectListeners() {
        ...
        let dataStore = this.data.getDataStore();
                ...
                if (target !== null) { //Network loaded etc.
                    ...
                    for (let i = 0; i < this.data.dataSets.length; ++i) {
                        if (this.data.dataSets[i].name === target.value && i !== this.data.dataIndex) {
                            ...
                            if (DEBUG_EVENTS)
                                console.log("EVENTS. Data set change. New set name: " + target.value);
                            return;
                        }
                    }
        ...
        const pValueSelect = DomUtil.getSelect(P_VALUE_SELECT);
        //Add complete list of p-values
        const pValueArray = TTest.getOneTailPValues();
