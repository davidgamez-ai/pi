...
export class SequenceData {
     ...
     * @param numSteps
     */
    buildSequenceData(numSteps) {
        //Extract the values and discard timestamp
        const values = this.dataSet.tsData.map(d => (d.value));
        //Split data
    ...
    getTrainTensors(trainPercent) {
        ...
        let trainLength = Math.floor(this.items.length * (trainPercent / 100));
        if (DEBUG_DATA)
            console.log("Data length: " + this.items.length + "; train length: " + trainLength);
    ...
    getValidationTensors(trainPercent, validationPercent) {
        ...
        //Calculate indexes for split
        let trainLength = Math.floor(this.items.length * (trainPercent / 100));
        let valLength = Math.floor(this.items.length * (validationPercent / 100));
        if (DEBUG_DATA)
            console.log("Data length: " + this.items.length + "; val length: " + valLength);
        //Build train, validate and test tensors
    ...
    getTestTensors(trainPercent, validationPercent, testPercent) {
        ...
        if (DEBUG_DATA)
            console.log("Data length: " + this.items.length + "; train: " + trainLength + "; validation: " + valLength + "; test: " + inputTest.arraySync().length);
        //Return
     ...
     * machine learning.
     * */
    getIntelligenceMeasurementTensors() {
        ...
        if (DEBUG_DATA)
            console.log("Items for intelligence measurement. Data length: " + inputIntelMeasure.arraySync().length);
        //Return
     ...
     * @param seqDataArr
     */
    getLabelTensor(seqDataArr) {
        //tf.tidy executes the function and cleans up intermediate tensors to prevent memory leaks
        return tf.tidy(() => {
            //Split out the labels
            const labels = seqDataArr.map(d => (d.nextValue));
